...
export const ImagerDisplay = d => {
  ...
  return (
    <div
      id='imagerDisplayId'
      className='imagerDisplay'
      style={{ zIndex }}
      dis='false'
    >
      <div
        className='imagerBackground'
        onMouseDown={mouseDownBackground}
        onMouseUp={mouseUpBackground}
      ></div>
...
const imagerShowFromImg = e => {
  ...
  animate(
    true,
    [boundig.left + boundig.width / 2, boundig.top + boundig.height / 2],
    [boundig.width, boundig.height]
  );
};
...
const waitElement = (el, property, value) =>
  new Promise(function repeat(resolve, reject) {
...
async function add(src, id, side) {
  ...
  document
    .getElementById('imagerDisplayId')
    .querySelector('.imagerList')
    .appendChild(wrap);
...
async function usualHide() {
  ...
  const img = display.querySelector(
    `.imagerList .imagerWrapImg[side='center']`
  );
...
async function animate(show, coord, originSize) {
  ...
  const list = display.querySelector('.imagerList');
  if (show) {
    display.setAttribute('coord', `${coord[0]},${coord[1]}`);
  }
  const img = display.querySelector(
    `.imagerList .imagerWrapImg[side='center']`
  );
  originSize = originSize === undefined ? [0, 0] : originSize;
  if (show && originSize[0] != 0) {
    display.setAttribute('originSize', `${originSize[0]},${originSize[1]}`);
  }
  if (
    !show &&
    display.getAttribute('originSize') &&
    img.getAttribute('idimg') == '0'
  ) {
    originSize = display.getAttribute('originSize').split(',');
    for (let i in originSize) {
      originSize[i] = parseInt(originSize[i]);
    }
  }
...
window.addEventListener('keydown', e => {
  ...
  if (e.code === 'ArrowLeft') {
    if (ind > 0) {
      const prev = display.querySelector(
        `.imagerList .imagerWrapImg[idimg='${ind - 1}']`
      );
      img.style.transform = 'matrix(1, 0, 0, 1, 0, 0)';
    ...
    if (ind < len - 1) {
      const next = display.querySelector(
        `.imagerList .imagerWrapImg[idimg='${ind + 1}']`
      );
      img.style.transform = 'matrix(1, 0, 0, 1, 0, 0)';
...
function imgMouseUp(e) {
  ...
  if (wrap.getAttribute('class') !== 'imagerWrapImg') {
    return;
  }
  const style = getComputedStyle(wrap);
  const matrix = style.transform.slice(7, -1).split(', ');
...
function imgMouseMove(e, delta) {
  ...
  for (let i in translate) {
    translate[i] = parseFloat(translate[i]);
  }
  ...
  const limit = [
    window.innerWidth / 2 +
      (matrix[0] * parseFloat(style.width.slice(0, -2))) / 2 -
      500,
    window.innerHeight / 2 +
      (matrix[3] * parseFloat(style.height.slice(0, -2))) / 2 -
      500,
  ];
...
function mouseUpBackground(e) {
  if (
    ...
    e.target &&
    e.target.tagName &&
    e.target.getAttribute('class') == 'imagerBackground' &&
    e.target.getAttribute('hide') == 'true'
  ) {
    console.log('click modal');
    animate(false);
...
window.addEventListener('touchmove', e => {
  ...
  for (let i in startZoom) {
    startZoom[i] = parseFloat(startZoom[i]);
  }

  const currArrange = arrange(
    [e.touches[0].clientX, e.touches[0].clientY],
    [e.touches[1].clientX, e.touches[1].clientY]
  );
...
window.addEventListener('touchstart', e => {
  ...
  if (prev) {
    prev.style.transitionDuration = '0s';
  }
  if (next) {
...
window.addEventListener('touchend', e => {
  ...
  if (prev) {
    prev.style.transitionDuration = transition;
  }
  if (next) {
    next.style.transitionDuration = transition;
  }

  img.style.removeProperty('margin-left');
  if (prev) {
    prev.style.removeProperty('margin-left');
  }
  if (next) {
    next.style.removeProperty('margin-left');
  }
...
window.addEventListener('touchmove', e => {
  ...
  if (
    e.touches[0].clientY - start[1] < 0 &&
    Math.abs(e.touches[0].clientX - touchCoord) < 30
  ) {
    ...
    e.touches[0].clientY - start[1] < 50 &&
    Math.abs(e.touches[0].clientX - touchCoord) > 0
  ) {
    img.style.marginLeft = `${2 * (e.touches[0].clientX - touchCoord)}px`;
    if (prev) {
      prev.style.marginLeft = `${
        2 * (e.touches[0].clientX - touchCoord - window.innerWidth)
      }px`;
    }
    if (next) {
